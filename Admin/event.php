...
<form id="s" method="post" action="event.php">
...
<p><font size="3">Identify the name and event date of which you want to change the above value for :</p>
<input type="text" placeholder="type event name here.." name="updateValueDataName" size="18">
	...
	if (array_key_exists('reset', $_POST)) {
		...
		if (array_key_exists('insertsubmit', $_POST)) {
            ...
            if (array_key_exists('deleteAll', $_POST)) {
                executePlainSQL("delete from event");
                OCICommit($db_conn);
            } 
            else {
                if (array_key_exists('updateValueAction', $_POST) || array_key_exists('updateValue', $_POST)) {
                    ...
                    executeBoundSQL("update event set " . $_POST['updateValue'] . "= " . $updateValueDataGeneric . " where eventName= TO_CHAR(:bind3) and eventDate= TO_DATE(:bind4,'yyyy/mm/dd')", $alltuples);
                    OCICommit($db_conn);
                }
            }
        } 
